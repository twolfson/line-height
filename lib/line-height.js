...
function lineHeight(node) {
  ...
  if (lnHeightStr === 'normal') {
    // Attempt to retrieve the line height from the cache
    var nodeName = node.nodeName,
        fontSizeStr = computedStyle(node, 'font-size'),
        cacheName = nodeName + '>' + fontSizeStr;
    lnHeight = _lnHeightCache[cacheName];

    // If we haven't cached the line height, calculate it
    if (lnHeight === undefined) {
      ...
      // Save the lnHeight to our cache
      _lnHeightCache[cacheName] = lnHeight;
    }
  }
