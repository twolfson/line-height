...
function lineHeight(node) {
  ...
  var lineHeightStr = computedStyle(node, 'line-height'),
      lnHeight = Math.round(parseFloat(lineHeightStr, 10));

  console.log('x', lnHeight);

  // If the line-height is "normal", calculate by font-size
  if (lineHeightStr === 'normal') {
    // If we don't yet have our ratio, calculate it
    var _ratio = lineHeight._ratio;
    if (_ratio === undefined) {
      ...
      console.log('zzz', _fontSizeStr, _fontSize);

      // Save our ratio
      _ratio = _height / _fontSize;
      lineHeight._ratio = _ratio;
